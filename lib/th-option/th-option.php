...
class portfoliolite_theme_option{
...
function menu_tab() {
    ...
    add_theme_page( esc_html__( 'Portfoliolite', 'portfoliolite' ), $menu_title, 'edit_theme_options', 'thunk_started',array($this,'tab_page'));
...
wp_enqueue_script('portfoliolite-admin-load', get_template_directory_uri() . '/lib/th-option/assets/js/th-options.js',array( 'jquery', 'updates' ),'1', true);
...
$data = apply_filters(
    ...
    wp_localize_script( 'portfoliolite-admin-load', 'THAdmin', $data); 
...
function tab_constant(){
    ...
    $tab_array = array();
    $tab_array['header'] = array('theme_brand' => __('ThemeHunk','portfoliolite'),
    'theme_brand_url' => esc_url($theme_data->get( 'AuthorURI' )),
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'portfoliolite'), esc_html__($theme_data->get( 'Name' )), $theme_data->get( 'Version' ) ),
    'welcome_desc' => esc_html__($theme_data->get( 'Name' ).' theme is not just a portfolio theme but a multipurpose theme.', 'portfoliolite' ),
...
if(empty($pages)){
      $post_id = wp_insert_post(array (
       'post_type' => 'page',
       'post_title' => __('Home Page','portfoliolite'),
       'post_content' => '',
       'post_status' => 'publish',
...
function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a class="plugin-detail thickbox open-plugin-details-modal" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'Details & Version', 'portfoliolite' ).'</a>
            </div>';
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            $button_class = 'install-now button '.$slug;
             ...
             if ( is_plugin_active( $plugin_init ) ) {
                   ...
                   $button_txt = esc_html__( 'Activated', 'portfoliolite' );
...
$boj = new portfoliolite_theme_option(); ?>